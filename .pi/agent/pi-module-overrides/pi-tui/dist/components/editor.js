...
export class Editor {
    ...
    render(width) {
        ...
        // Add autocomplete list above the prompt so suggestions do not push the editor off the bottom.
        ...
        const boxStart = this.autocompleteState && this.autocompleteList ? this.autocompleteList.render(contentWidth).length : 0;
        if (width >= 2 && result.length - boxStart >= 2) {
            ...
            result[boxStart] = this.borderColor(`${topLeft}${"─".repeat(topFill)}${label}${topRight}`);
            result[result.length - 1] = this.borderColor(`╰${"─".repeat(Math.max(0, width - 2))}╯`);
            for (let i = boxStart + 1; i < result.length - 1; i++) {
                const inner = truncateToWidth(result[i], Math.max(0, width - 2), "");
                const pad = " ".repeat(Math.max(0, width - 2 - visibleWidth(inner)));
                result[i] = `${this.borderColor("│")}${inner}${pad}${this.borderColor("│")}`;
            }
        }
        return result;
    }
