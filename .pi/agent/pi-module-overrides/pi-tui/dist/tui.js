...
export class TUI extends Container {
    ...
    setFixedBottomCount(count) {
        this.fixedBottomCount = Math.max(0, Math.floor(count));
        this.requestRender(true);
    }
    render(width) {
        if (this.fixedBottomCount <= 0 || this.children.length <= this.fixedBottomCount) {
            return super.render(width);
        }
        const split = this.children.length - this.fixedBottomCount;
        const normalChildren = this.children.slice(0, split);
        const fixedChildren = this.children.slice(split);
        const normalLines = [];
        for (const child of normalChildren) {
            normalLines.push(...child.render(width));
        }
        const fixedLines = [];
        for (const child of fixedChildren) {
            fixedLines.push(...child.render(width));
        }
        const available = Math.max(0, this.terminal.rows - fixedLines.length);
        const visibleNormal = normalLines.length > available
            ? normalLines.slice(normalLines.length - available)
            : normalLines;
        const padding = Math.max(0, available - visibleNormal.length);
        return [...visibleNormal, ...Array(padding).fill(""), ...fixedLines];
    }
