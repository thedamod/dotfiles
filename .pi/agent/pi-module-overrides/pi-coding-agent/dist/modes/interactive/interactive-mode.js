...
export class InteractiveMode {
    ...
    async init() {
        ...
        // Bind the lightweight initial session immediately; heavy resources can load after first paint.
        await this.rebindCurrentSession();
        // Render initial messages AFTER showing loaded resources
        this.renderInitialMessages();
        if (process.env.PI_RESOURCES_DEFERRED === "1") {
            delete process.env.PI_RESOURCES_DEFERRED;
            setTimeout(() => {
                void this.loadDeferredResources();
            }, 0);
        }
    ...
    async loadDeferredResources() {
        try {
            this.showStatus("Loading extensions, skills, prompts, and themes...");
            await this.session.reload();
            await this.rebindCurrentSession();
            this.showStatus("Loaded extensions, skills, prompts, and themes");
            this.ui.requestRender();
        }
        catch (error) {
            this.showError(`Deferred resource load failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
    ...
    updateEditorBorderColor() {
        ...
        const labelParts = [];
        if (model) {
            labelParts.push(`${model.provider}/${model.id}`);
        }
        if (level && level !== "off") {
            labelParts.push(level);
        }
        this.editor.setStatusLabel?.(labelParts.join(" · "));
        this.ui.requestRender();
    }
    cycleThinkingLevel() {
