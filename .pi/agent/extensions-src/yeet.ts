...
function parseSelectedModel(value: string) {
...
function buildYeetPrompt(extraInstructions: string) {
  return [
...
export default function yeet(pi: ExtensionAPI) {
  let rememberedModel: { provider: string; id: string } | undefined;
  ...
  pi.registerCommand("yeet", {
    ...
    handler: async (args, ctx) => {
      ...
      const inside = await new Promise<boolean>((resolve) => {
      ...
      if (!model) {
        ...
        const selected = await ctx.ui.select(
          "Choose model for /yeet commit planning",
          current ? [current, ...labels.filter((label) => label !== current)] : labels,
        );
